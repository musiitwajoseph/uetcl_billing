...
Class FeedBack{
	...
	function withCents($amount, $currency=" Shillings"){		
		$totalCals = explode('.', $amount);
		if(count($totalCals)>=2 && ($currency==' Dollars'||$currency == ' USD')){
			$x = $totalCals[1];
			$x = str_replace(',','',number_format('0.'.$x,2));
			$x *= 100;
			$x = str_pad($x, 2, 0, STR_PAD_LEFT);
			//$x = '06';
			$currency = " ".$currency." And ".Feedback::number_to_words($x)." Cents";
		}else{$currency = "";}
		return Feedback::number_to_words($totalCals[0]).' '.$currency;
	}
	function refresh($seconds = 1, $link = ""){
		if($link == ""){
	...
	function status($start_date, $end_date, $id=1){
		$time = time();
		$db = new Db();
		...
		$p = strtotime("+$fs_qty months", time());

		if(time() > $end_date){
			return true;
			//$a = "<span style='color:red;font-weight:bold;'>Contract Expired, Needs Renewal</span>";
		}else if($p > $end_date){
			return false;
			//$a = "<span style='color:red;font-weight:bold'>Contract is about to expire</span>";
		}
			
		return $a;
	}
	
	function status2($start_date, $end_date, $id=1){
		$time = time();
		$db = new Db();
		$select = $db->select("SELECT * FROM fibre_service WHERE fs_customer_id = '$id' ");
		extract($select[0][0]);
	...
	function minuteDifference($from) {
	...
	function date_tr($time){
		return "".date("M dS Y", $time);
	} 
	
	function date_r($time){		
		return "".date("tS M 'y",strtotime(date("t M Y", $time)));
	}

	function date_c($time){		
		return "".date("dS M 'y",strtotime(date("d M Y", $time)));
	}
	...
	function sendmail($to,$subject,$message,$name, $telephone="+256788229210", $attachments = array()){
    ...
    function sendmail2($to,$subject,$message,$name, $telephone="+256788229210"){
		...
		$mail->SMTPSecure = 'tls';
		$mail->SetFrom('josemusiitwa@gmail.com', 'UETCL');
		$mail->AddReplyTo("josemusiitwa@gmail.com","UETCL");
		$mail->Subject    = $subject;
		$mail->AltBody    = "To view the message, please use an HTML compatible email viewer!";
		$mail->MsgHTML($body);
		$address = $to;
    ...
    function sms1($message, $number="+256788229210"){ echo 'good'; return 0; }

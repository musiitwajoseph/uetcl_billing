				...
				//if(1)
			{
				if ($_FILES[$file]["error"] > 0){//errors
					...
					if (file_exists($path.$folder_name.'/' . $_FILES[$file]["name"])){
						...
						$uploaded_file = strtolower($path.$folder_name.'/'.time().'-'.basename(str_replace(' ', '_', $_FILES[$file]['name'])));

						if(move_uploaded_file($_FILES[$file]['tmp_name'],$uploaded_file)){
							$uploaded_file;
							//paths to resize and to store the resized image
							$this->image_link = strtolower($folder_name.'/'.time().'-'.basename(str_replace(' ', '_', $_FILES[$file]['name'])));
							$this->error = 'Uploaded';
							$this->is_error = true;
						}else{

...
if(1){
	...
	$is = str_replace(",","",$_POST['is']);
	$ip = str_replace(",","",$_POST['ip']);
	$io = str_replace(",","",$_POST['io']);
	$es = str_replace(",","",$_POST['es']);
	$ep = str_replace(",","",$_POST['ep']);
	$eo = str_replace(",","",$_POST['eo']);
	$mp_id = str_replace(",","",$_POST['mp_id']);
	$rea_id = $_POST['rea_id'];
	$year = $_POST['year'];
	$month = $_POST['month'];
	...
	$rate__id = $_POST['rate__id'];
	$comment__id = $_POST['comment__id'];
	...
	if(empty($comment)){
		...
		$select = $db->update("readings_comment", ["rc_comment"=>$comment, "rc_date_added"=>time(), "rc_added_by"=>$user_id, "rc_customer_id"=>$customer_id, "rc_year"=>$year, "rc_month"=>$month, "rc_mp_id"=>$mp_id, "rc_type"=>$portion],["rc_id"=>$comment__id]);
		...
		for($i=0; $i<count($x); $i++){ 
			...
			if($$y != ""){
				...
				$select = $db->update("r_rate", $a, ["rate_cus_id"=>$customer_id, "rate_id"=>$rate__id]);
			}
		}
		...
		//FeedBack::redirect(return_url()."/".portion(1)."/".$portion."/".portion(3)."/".portion(4)."/".portion(5)."#$rea_id");
			...
			if(1){ //imports as advance
				$select = $db->update("r_rate", $aaa, ["rate_cus_id"=>$customer_id, "rate_id"=>$rate__id]);

...
echo '<h5>'.$name.'</h5>';
if($db->num_rows()){
	$select = $db->select("SELECT * FROM invoice WHERE inv_from = '$factory' AND (inv_consignment_number IS NOT NULL or inv_consignment_number != 0) AND inv_type IS NULL");
	//echo $db->num_rows();
	if($db->num_rows()){
				...
				$('#invNumber').change(function(){				
					...
					if($('#invNumber').val()!=""){
						$.ajax({
				            ...
				            url: '<?php echo return_url().'ajax/export_invoice_details_contract.php'; ?>',
				            type: "POST",
				            data: form_data,
				            contentType: false,
				            cache: false,
